 ...
 * 4. 路由模块间的数据（如：数据编辑器 -> 绘图 -> 拟合）
 * 5. 负责统一弹出项目操作成功的提示框，并应用统一样式。
 */
...
#include "wt_plottingwidget.h" // 引用新的图表头文件
...
void MainWindow::onProjectClosed()
{
    ...
    if (m_DataEditorWidget) {
        // m_DataEditorWidget->clear();
    }

    // 2. 重置图表界面
    // if (m_PlottingWidget) { m_PlottingWidget->clear(); }
...
void MainWindow::transferDataFromEditorToPlotting()
{
    ...
    // 将数据模型传递给新的图表控件
    // 新的控件会自动处理模型，用户在界面上点击新建曲线时再读取数据
